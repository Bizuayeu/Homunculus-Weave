...
CorporateStrategistは、**柔軟なサブスキル選択**により、トークン消費を最適化します。
...
ステップ1: スキル起動
  ユーザーが要求を伝える
    ↓
ステップ2: サブスキル推定/選択
  Claudeが要求を分析し、適切なサブスキルを提案（パターンA）
  または選択肢を提示（パターンB）
    ↓
ステップ3: ユーザー確認
  ユーザーが承認または追加/変更を指示
...
ユーザー: 「事業モデルを整理したい」
    ↓
CorporateStrategist: 「BusinessAnalyzer（事業分析）で対応します。よろしいですか？」
    ↓
ユーザー: 「OK」
    ↓
...
ユーザー: 「人事について相談したい」
    ↓
CorporateStrategist: サブスキル選択肢を提示
    ↓
...
ユーザー: 「採用判断をして、契約書も作成したい」
    ↓
CorporateStrategist: 「PersonnelDeveloperとLegalAdviserで対応します。よろしいですか？」
    ↓
ユーザー: 「OK」
    ↓
...
CorporateStrategistの強みは、4つのサブスキルが相互に連携することです。

### 連携例1: 採用判断の総合支援

```
PersonnelDeveloper: 採用前判断（外注QCD比較）
    ↓ 連携
BusinessAnalyzer: 事業戦略との整合性確認
    ↓ 連携
LegalAdviser: 雇用契約書の作成
    ↓ 連携
ForesightReader: 候補者の適性判断
```

### 連携例2: 組織再編の戦略立案

```
BusinessAnalyzer: 事業構造の分析
    ↓ 連携
PersonnelDeveloper: 人材配置の最適化
    ↓ 連携
ForesightReader: 人材の適性と相性の分析
    ↓ 連携
LegalAdviser: 組織変更の法的確認
```
...
**A**: CorporateStrategist起動時に、選択肢が提示されます（パターンB）。各サブスキルの説明を見て、必要なものを選択してください。
...
CorporateStrategistに、やりたいことを自然言語で伝えてください。

例：
- 「営業事務を採用すべきか判断したい」
- 「事業構造を整理して、人材配置を最適化したい」
- 「雇用契約書を作成したい」
- 「候補者の適性を判断したい」

### ステップ2: サブスキル選択の確認

CorporateStrategistが要求を分析し、適切なサブスキルを提案します（パターンA）。
要求が不明確な場合は、選択肢を提示します（パターンB）。

### ステップ3: 選択的読み込みと実行

ユーザーが確認/選択したサブスキルのみを読み込み、専門的な支援を提供します。
