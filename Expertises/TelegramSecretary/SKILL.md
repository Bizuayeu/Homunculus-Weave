...
| `individuals\|tasks\|knowledge {list\|get\|add\|remove}` | 管理表（INDIVIDUALS/TASKS/KNOWLEDGE）の CRUD。`get`/`remove` は `--key`（uuid/id）、`add` は `--json`/`--json-file`。値オブジェクトで検証。SSoT は Private JSON、操作主体は SecretaryRole、入口は `/telegram-secretary` | 0=OK, 2=不正入力 |
