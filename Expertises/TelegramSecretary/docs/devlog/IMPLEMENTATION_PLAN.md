...
## Stage 11.5: PDF 判定撤廃＋オンデマンド抽出（`render-pdf`）と Live E2E

> **設計転換（2026-05-31）**: 当初の Stage 11.5 は「Stage 11（テキスト層判定の二経路）の Live E2E」だったが、二経路の誤判定リスク（スタンプ・薄いテキスト層で text 経路に落ち中身が読めない）を構造的に排除するため、**PDF を常に画像化**へ一本化し、テキスト全文・cap 超ページを `render-pdf` サブコマンドで **オンデマンド**に分離する設計へ転換した。`render()` は画像専用（判定削除）、`extract_text()` / `rasterize_pages()` を分離、`main.py` に `render-pdf --text|--pages` を追加。全 suite green=369 passed（358→+11、判定二経路テスト撤去分を相殺）。下記の旧 Documentation Plan / Decision Notes は Stage 11（二経路）時点の経緯として保持。

**Goal**: PDF は常に画像化（先頭 cap 枚）→ emit に `derived_image_paths` + `page_count` → Weave が **先頭最大 5 枚 Read → 大枠と総ページ把握 → ①全文 `render-pdf --text` ／ ②個別ページ（N≤20 はディスク済み、N>20 は `render-pdf --pages`）／ ③十分 を判断** する運用を Cloud Routine 上で成立させる。SKILL.md に PDF 仕様を集約（SSoT）、ROUTINE_PROMPT は手順・コマンド例に絞る（重複解消）、CHANGELOG / README を Doc Complete に。
**Layer**: Infrastructure（運用統合）
...
**Status**: 実装完了＋Doc Complete（2026-05-31。判定撤廃＋オンデマンド抽出：`render()` 画像専用化／`extract_text()`／`rasterize_pages()`／`render-pdf --text|--pages`。全 suite green=369 passed（358→+11）。CHANGELOG [0.10.0] / README（PDF Quickstart を render-pdf 運用へ + env + tests 369）/ SKILL（「PDF の扱い」SSoT 新設 + Subcommands + env + Security）/ ROUTINE_PROMPT（スキーマ + render_status + media 処理を新仕様＋コマンド例へ、SKILL 参照に重複解消）すべて着地。**Live E2E（実 Telegram で PDF → 最大 5 枚 Vision → render-pdf 往復、retention）は 2026-05-31 に全5ケース PASS**（CHANGELOG [0.10.1]、Cloud Routine 実機 `telegram-secretary-stage11.5-e2e-pdf-ondemand`。Case 4 の cap 超 58p 地盤調査報告書で `render-pdf --pages` オンデマンド生成が実文書稼働、漏洩スキャン全クリーン））
...
**Status**: 実装完了（2026-05-30 Stages 11.1〜11.4 コード Red→Green=358 passed。**2026-05-31 Stage 11.5 で判定二経路を撤廃し PDF 常時画像化＋`render-pdf` オンデマンドへ設計転換、369 passed・回帰ゼロ**。Doc は SKILL（SSoT 集約）/ ROUTINE（手順・コマンド例）/ CHANGELOG [0.10.0] / README 着地。**Live E2E（最大 5 枚 Vision → render-pdf 往復）は 2026-05-31 全5ケース PASS**（CHANGELOG [0.10.1]）。pypdfium2/Pillow 直接宣言＝再現性重視。3-Strike #1 pypdfium2 API・#2 Pillow を実機検証で解消。OCR 三択から Vision 経路へ設計確定 2026-05-30）
