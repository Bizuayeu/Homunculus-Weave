...
- **`render_status` 四状態**（Stage 7/9/10）:
  - `"ok"` — md 化（docx/pptx/xlsx/html、Stage 7）**／ PDF テキスト層抽出（Stage 10 pdfplumber、Read tool 非依存）／ 音声 transcript（voice/audio/video の音声トラック、Stage 9 Moonshine）**成功、`rendered_text` 非 null。`kind`/`mime_type` で md か PDF 本文か transcript か判別。スキャン PDF 等テキスト層ゼロは `rendered_text=""`（読めるテキスト無し）
