...
**Status**: 実装完了（2026-05-30 Stages 11.1〜11.4 コード Red→Green=358 passed。**2026-05-31 Stage 11.5 で判定二経路を撤廃し PDF 常時画像化＋`render-pdf` オンデマンドへ設計転換、369 passed・回帰ゼロ**。Doc は SKILL（SSoT 集約）/ ROUTINE（手順・コマンド例）/ CHANGELOG [0.10.0] / README 着地。**Live E2E（最大 5 枚 Vision → render-pdf 往復）は fresh session 待ち**。pypdfium2/Pillow 直接宣言＝再現性重視。3-Strike #1 pypdfium2 API・#2 Pillow を実機検証で解消。OCR 三択から Vision 経路へ設計確定 2026-05-30）
