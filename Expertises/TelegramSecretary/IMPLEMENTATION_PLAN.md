...
## Stage 11（候補・backlog）: スキャン PDF OCR

> 追加日: 2026-05-30。Stage 10.4 Live E2E の P3 で**実需が実証された**（スキャン PDF を送った大環主が follow-up で OCR を明示要求）。ただし頻度は未知ゆえ **YAGNI で着手は保留**、調査結果のみ記録（再調査コストが高いため）。実務でスキャン PDF が頻発したら着手判断する。

**問題**: テキスト層ゼロのスキャン PDF は pdfplumber が `ok`+空を返す（Stage 10 で確定の正しい挙動）。中身（画像化された文字）を読むには OCR が要る。
...
**実装方針（着手時）**: PdfRenderer の空テキスト層検知 → ページ画像化（pdfplumber `page.to_image()` or pypdfium2）→ RapidOCR。MediaRenderer Port は不変ゆえ既存 Stage 10 枠に乗る。Stage 9.6-ii（key frame Vision の複数 media emit 拡張）と OCR は「画像→認識」で基盤を共有でき、統合実装も一案。

**Status**: Not Started（backlog、実需頻度を見て判断）
