...
> **方針**: 本ドキュメントは **SSoT を意図的に例外化**し、網羅性を優先する。配布物として単体で読んで穴が無いことが最優先のため、他ドキュメント（DESIGN / 上位 SECURITY.md / 開発記録）と内容が重複してもよい。配布時にセキュリティの穴があることの方が、重複より遥かに有害。
>
> 凡例 — ✅ 実装済み（テスト有） / 📋 計画（未実装、該当 Stage で実装） / ⚠️ 運用上の注意

## 脅威モデル概観

TelegramSecretary は **Cloud Routine 上で常駐し、外部（Telegram）からのメッセージを受けて エージェント が応答する**。攻撃面は以下:
...
- **添付生成物の漏洩スキャン**（Stage 8）— 送り返す md/docx/画像/PDF にも機密が混入していないか確認。コードはバイナリ中身まで検査しない＝エージェントの判断責務（LLM 判断をコード外に出す分業）
...
- **render 寛容性の認識**（Stage 7）— markitdown は garbage でも何か返す。rendered_text が意味あるテキストかは エージェント が判断（最終防御は エージェント 層）
...
- **shared_with 境界**（LINE 連携など複数チャネル併用時）— 関係者間の情報共有は `identity.shared_with` の明示許可制。未承認の relay は拒否し、`<OWNER>`（principal）に承認伺い
...
- [ ] `.gitignore` に開発中ディレクトリ（`LineBridge/` 等）と `state/`（実データ）が入っているか
- [ ] token redact テストが green か（network error 経路含む）
- [ ] injection_flags / 出力漏洩スキャンの運用が ROUTINE_PROMPT に明記されているか
- [ ] 配布ドキュメント（README/SKILL/SECURITY/STRUCTURE/ROUTINE_PROMPT/DESIGN）に固有名（人格名・運用主体名・組織名・Loop 参照・ローカル絶対パス）が残っていないか（grep 検査）
- [ ] プレースホルダ（`<AGENT_NAME>` / `<OWNER>` / `<ORGANIZATION>` / `<REPO_ROOT>` / `<PRIVATE_DIR>` / `<INSTALL_DIR>`）が規約どおり使われているか（[STRUCTURE.md](./STRUCTURE.md) のプレースホルダ規約）
- [ ] 開発記録（`docs/devlog/`）が配布パッケージから除外されているか（教材として別管理）
