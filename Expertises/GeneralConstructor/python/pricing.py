...
敷地小規模の上限 = Decimal("80")  # 2026-08-24 裁定で Excel の 70㎡ から変更（inference と共有）
車両判定の敷地下限 = Decimal("150")  # inference.vehicle_class と共有
車両判定の幅員境界 = Decimal("4")  # 4m 未満は立米車、4m 以上は中型車
_六層 = 6  # 「6層」行の対象層数
...
_適用条件 = {
    ...
    "敷地小規模（敷地面積80㎡以下）": lambda i, k: i.有効宅地面積 <= 敷地小規模の上限,
    "敷地面積150㎡以上で立米車": lambda i, k: (
        i.有効宅地面積 >= 車両判定の敷地下限 and i.前面道路幅員 < 車両判定の幅員境界
    ),
