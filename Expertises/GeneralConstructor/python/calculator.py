...
単価表の数字は税込なので、金額は全て税込で通す（2026-08-24 裁定。×1.1 の併記は退役）。
...
def calculate_basement_relaxation_area(
    ...
    全地下も半地下と同式（緩和あり）。地下床の容積率緩和という趣旨から、概算モデル
    としてはこれで足りる（2026-08-24 裁定で確定）。
    """
    if 半地下有無 == "半地下無":
        return Decimal("0")
...
def calculate_project_total(
    ...
    """PJ総額（税込）= 土地価格 + 工事代金 + 建設経費

    単価表の数字が税込なので、ここで ×1.1 はしない（2026-08-24 裁定。
    板橋 case_001 の最終単価 57.05 万円/㎡ が対外表示の「平米 57 万円強・税込」と合う）。
    """
...
def calculate_surface_yield(
    ...
    """表面利回 = 年間売上 / PJ総額 × 100（PJ総額ベース。単価表が税込ゆえ税込基準）"""
